/*
 * Q-Free's mark is inlined rather than loaded as an image because it is the one
 * two-tone logo in the set: a single red body with five white counters layered
 * on top of it. A CSS filter cannot flatten that -- greyscale washes the red to
 * pale grey and leaves the five knockouts white, so on cream paper the mark
 * loses its counters and reads as a rendering bug. Binding both tones to theme
 * tokens instead lets the counters follow the page in either theme.
 *
...
interface QFreeMarkProps {
    className?: string;
    // Empty (the default) marks the SVG decorative, matching how LogoMark treats
    // raster marks: the company name is already rendered as text beside it.
    label?: string;
}

const QFreeMark: React.FC<QFreeMarkProps> = ({ className, label = '' }) => (
    <svg
